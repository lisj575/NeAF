...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
                 ...
                 point_count_std=0.0, sparse_patches=False, neighbor_search_method='r', query_vector_path='./query_vector_5k.xyz', batch_query_size=512):
        ...
        # load qeury vectors
        if os.path.exists(query_vector_path):
            self.query_vectors = np.loadtxt(query_vector_path).astype('float32')
            self.query_vector_num = self.query_vectors.shape[0]
        else:
            raise ValueError('No such query vector file: %s' % (query_vector_path))
    ...
    def __getitem__(self, index):
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            ...
            ###plane_score[start:end] = torch.norm(patch_pts[start:end, :]*torch.from_numpy(shape.normals[center_point_ind:center_point_ind+1,:]),dim=-1,p=2)
        ...
        #print("centering time: %fs" %(1000*(start_time-end_time)))
        if self.include_normals:
        ...
        # loading query_vectors
        
        sample_ind = self.rng.choice(self.query_vector_num, size=self.batch_query_size, replace=False)
        query_vectors = torch.tensor(self.query_vectors[sample_ind, :])
        angle_offsets = compute_angle_offset(query_vectors, patch_normal)
        # start_time = time.time()
        if self.use_pca:
            ...
            query_vectors =  torch.matmul(query_vectors, trans)
        ...
        return (patch_pts,) + patch_feats + (trans,)  + (patch_scale,) + (shape.pts[center_point_ind, :], ), (query_vectors, angle_offsets)
...
def compute_angle_offset(query_vector, gt_normal): # query_vector: batchsize * 3, gt normal batchsize * 3
