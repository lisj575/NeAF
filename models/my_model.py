...
from models.netBase import BaseBC,BaseBF
# import math
...
class DeepFit(nn.Module):
    def __init__(self, k=1, num_points=500, use_point_stn=False,  use_feat_stn=False, point_tuple=1,
                 ...
                 use_consistency=False, use_batchNormalization=True, use_wn=True):
        ...
        self.use_bn = use_batchNormalization
        if not use_wn:
            self.decoder = Decoder(latent_size=128, output_dim=1, weight_norm=use_wn, norm_layers=None)
        else:
            self.decoder = Decoder(latent_size=128, output_dim=1)

    def forward(self, points, query_vectors):
        ...
        patch_feature = torch.max(x, dim=2)[0] # batchsize *128   
        feature = patch_feature.repeat(1, query_vectors.shape[1]).reshape((query_vectors.shape[0], query_vectors.shape[1], 128)) # batchsize * normal_size * dim

        rot_query = torch.matmul(query_vectors, trans)

        query_feat = self.norm_encoder(rot_query) # batchsize * n_norm * 128
        condition = torch.cat((feature, query_feat),2) #batchsize * n_norm * (128 + 128) 
        theta = self.decoder(condition).squeeze(-1)

        return theta, trans, trans2
...
class Decoder(nn.Module):
    def __init__(
        ...
        dims = [latent_size + norm_size] + dims + [output_dim]
    ...
    def forward(self, input):
        xyz = input[:, :, -self.norm_size:]
        if input.shape[2] > self.norm_size and self.latent_dropout:
            latent_vecs = input[:, :, :-self.norm_size]
            latent_vecs = F.dropout(latent_vecs, p=0.2, training=self.training)
            x = torch.cat([latent_vecs, xyz], 2)
        else:
            x = input
